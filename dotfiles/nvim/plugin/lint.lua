...
local function find_eslint_config(filename)
  local found_child = vim.fs.find(config_files, {
    path = filename,
    upward = false,
  })
  if found_child and #found_child > 0 then
    return found_child[1]
  end

  local found_parent = vim.fs.find(config_files, {
    path = filename,
    upward = true,
    stop = vim.loop.os_homedir(),
  })
  if found_parent and #found_parent > 0 then
    return found_parent[1]
  end
